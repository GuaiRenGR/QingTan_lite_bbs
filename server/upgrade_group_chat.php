...
try {
    ...
    try { $pdo->exec("ALTER TABLE `{$prefix}chat_groups` ADD COLUMN `announcement` TEXT NULL AFTER `name`"); } catch (Throwable $e) {}
    try { $pdo->exec("ALTER TABLE `{$prefix}chat_group_members` ADD COLUMN `muted_until` DATETIME NULL AFTER `role`"); } catch (Throwable $e) {}
